// PsPaint メインクラス
var PsPaint = function(options){
    this.version = 1.04;	// バージョン
    ...
    this.currentBrushSize = 2; // size 1
    ...
    this.undoBuffer = new (function(depth){
        ...
        this.pushUndo = function(layers, layerNum, area){
        	...
        	var a;
			try{
				a = ctx.getImageData(left, top, width, height);
				bctx.putImageData(a, left, top);
			}catch( e ){
			  //alert( e );  // ダイアログ表示
			}
...
// マウス操作系イベント up/down/move/out
PsPaint.prototype.onMouseEvents = function(e){
...
// キーイベント
PsPaint.prototype.onKeyEvents = function(e){
...
// 初期化
PsPaint.prototype.clearAllLayers = function(){
    if(window.confirm("全てのレイヤーのデーターが削除されます。また、この操作は取りせません。")){
...
PsPaint.prototype.resoreCanvas = function(){
    var canvas = this.layer.getCanvas(this.curentLayer);
...
// Set current color
PsPaint.prototype.setColor = function(color){
...
// Set brush size
PsPaint.prototype.setBrushSize = function(size){
    this.currentBrushSize = size;
};

// Select current layer
PsPaint.prototype.selectLayer = function(number){
...
PsPaint.prototype.fireCanUndoEvent = function(){
...
PsPaint.prototype.restoreTool = function(){
    var currentTool = this.currentTool;
...
PsPaint.prototype.fireToolChangedEvent = function(){
...
// Get the layer
PsPaint.prototype.getLayer = function(n){
    return this.layer.getCanvas(n);
};

// Get the integrated layer
PsPaint.prototype.getIntegratedLayer = function(){
    return this.layer.getIntegratedLayer();
}

// Get the image file as the new window
PsPaint.prototype.getImageToWindow = function(){
...
// Hide the layer
PsPaint.prototype.hideLayer = function(n){
    this.fireLayerVisible(n, false);
    return this.layer.hideCanvas(n);
};

// Show the layer
PsPaint.prototype.showLayer = function(n){
    this.fireLayerVisible(n, true);
    this.layer.showCanvas(n);
};
// Toggle the layer
PsPaint.prototype.toggleLayer = function(n){
...
PsPaint.prototype.fireLayerVisible = function(n, visibility){
...
var painter;
$(function(){
    var paperPosition = $("#paper").offset(); // 表示位置を返す
    var drawArea = { //お絵描きエリア
        left: paperPosition.left + 1,
        top: paperPosition.top + 1,
        width: $("#paper").width(),
        height: $("#paper").height()
    };
    painter = new PsPaint({canvas: $("div#eventcatcher").get(0), drawArea : drawArea});
});
...
//メニュー部分
    $("#button_file_upload").click(function(){
		...
		image.onload = function() {
			// 画像を描画
			//var canvas = this.layer.getCanvas(this.curentLayer);
			var canvas = painter.layer.getCanvas(painter.curentLayer);
			ctx = canvas.getContext("2d");
			ctx.drawImage(image, 0, 0);
		};
		// 画像のURLをセットし、読み込み開始
		image.src = "file:///C:/Documents%20and%20Settings/nobuseki/My%20Documents/labo/html5paint/sb.jpg";
        //painter.popUndo();
        return false;
    });
    $("#button_add_layer").click(function(){
    	alert('a');
		//var canvas = painter.layer.getCanvas(painter.curentLayer);
    	//var $canvas = $(canvas);
    	//$canvas.css({position:"absolute", left:50, top:50, zIndex: 90 - 1 * 2});

    	//レイヤー増やす
    	painter.layer.layerLength = painter.layer.layerLength+1;
    	painter.buffers.push();
